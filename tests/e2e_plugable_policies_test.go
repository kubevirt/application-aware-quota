...
var _ = Describe("ApplicationAwareQuota plugable policies", func() {
	...
	Context("both label-sidecar and annotation-sidecar evaluators", func() {
		ctx := context.Background()
		BeforeEach(func() {
			currAAQ, err := utils.GetAAQ(f)
			Expect(err).ToNot(HaveOccurred())
			if libaaq.CheckIfPlugablePolicyExistInAAQ(currAAQ, libaaq.LabelSidecar, libaaq.Double) &&
				libaaq.CheckIfPlugablePolicyExistInAAQ(currAAQ, libaaq.AnnotationAppAnnotation, libaaq.Double) {
				return
			}
			Eventually(func() error {
				aaq, err := utils.GetAAQ(f)
				Expect(err).ToNot(HaveOccurred())
				aaq.Spec.Configuration.SidecarEvaluators = []v1.Container{}
				aaq = libaaq.AddPlugablePolicy(aaq, libaaq.LabelSidecar, libaaq.Double)
				aaq = libaaq.AddPlugablePolicy(aaq, libaaq.AnnotationSidecar, libaaq.Double)
				_, err = f.AaqClient.AaqV1alpha1().AAQs().Update(context.Background(), aaq, v12.UpdateOptions{})
				return err
			}, 60*time.Second, 1*time.Second).ShouldNot(HaveOccurred(), "waiting for aaq policies update")
		...
		DescribeTable("Counting", func(addAppLabel bool, addAppAnnotation bool) {
			arq := builders.NewArqBuilder().WithName("test-arq").WithNamespace(f.Namespace.Name).
				WithResource(v1.ResourceRequestsMemory, resource.MustParse("4Gi")).Build()
			_, err := createApplicationAwareResourceQuota(ctx, f.AaqClient, f.Namespace.Name, arq)
			Expect(err).ToNot(HaveOccurred())
			...
			requests := v1.ResourceList{}
			requests[v1.ResourceMemory] = resource.MustParse("256Mi")
			pod := newAppsPodForQuota(podName, requests, addAppLabel, addAppAnnotation)
			pod, err = f.K8sClient.CoreV1().Pods(f.Namespace.Name).Create(ctx, pod, v12.CreateOptions{})
			Expect(err).ToNot(HaveOccurred())
			verifyPodIsNotGated(f.K8sClient, f.Namespace.Name, pod.Name)
			...
			if addAppLabel && addAppAnnotation {
				expectedCounting.Add(requests[v1.ResourceMemory])
			}
			...
			Entry(" should be quadruple for pods with both the app label and the app annotation", true, true),
			Entry(" should be double for pods with only the app label and without the app annotation", true, false),
			Entry(" should be double for pods with only the app annotation and without the app label", false, true),
			Entry(" should be regular for pods without the app label and without the app annotation", false, false),
		)
	})
})
