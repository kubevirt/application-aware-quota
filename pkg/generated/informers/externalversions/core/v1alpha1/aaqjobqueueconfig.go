...
// NewAAQJobQueueConfigInformer constructs a new informer for AAQJobQueueConfig type.
...
func NewAAQJobQueueConfigInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredAAQJobQueueConfigInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredAAQJobQueueConfigInformer constructs a new informer for AAQJobQueueConfig type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
...
func (f *aAQJobQueueConfigInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredAAQJobQueueConfigInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}
