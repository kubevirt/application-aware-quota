...
func GetAAQSubresourceClientFromFlags(master string, kubeconfig string) (AAQClient, error) {
	...
	generatedAAQClient, err := generatedclient.NewForConfig(config)
	if err != nil {
		return nil, err
	}

	kubevirtClient, err := kuebvirtclient.NewForConfig(config)
	if err != nil {
		return nil, err
	}
...
func GetAAQClientFromRESTConfig(config *rest.Config) (AAQClient, error) {
	...
	generatedAAQClient, err := generatedclient.NewForConfig(&shallowCopy)
	if err != nil {
		return nil, err
	}
...
func GetAAQClient() (AAQClient, error) {
	var err error
	once.Do(func() {
		aaqclient, err = GetAAQClientFromFlags(master, kubeconfig)
	})
	return aaqclient, err
}

func GetAAQSubresourceClient() (AAQClient, error) {
	return GetAAQSubresourceClientFromFlags(master, kubeconfig)
}
