...
`requests.cpu`, `limits.cpu`, `requests.memory`, `limits.memory`, and more. This architecture
wasn't designed to be easily extendable.
...
`limits.memory` boundaries in a namespace, then pods that don't set these resources are not
allowed to be created. This essentially means that in such namespaces, best effort pods and some
burstable pods are not allowed. The design decision which caused this limitation in Kubernetes 
is admitted to be a mistake, but is still kept for backward compatibility reasons as mentioned 
in the following
...
**User Story**: Creating non-guaranteed QoS Pods in a multi-tenant cluster

As a user in a multi-tenant Kubernetes cluster, I need to deploy best-effort pods for low-priority 
workloads, which should utilize spare cluster resources and be the first to terminate during shortages. 
...
For resource boundaries defined in the quotas, affected by pod creation, 
deletion, or changes—like `limits.memory`, `pods`, `requests.ephemeral-storage`, and others:

**Note**: These resources include not just the standard Kubernetes resources but also those
counted in Pluggable policies.

Refer to the "Pluggable policies for custom counting" section for more details.

1. Scheduling Gates: Scheduling gates are added to pods within namespaces where
   the pod's namespace match the namespaceSelector in the AAQ object.
   See the "Configure AAQ Operator" section below for more information
2. Controller Evaluation: Once a pod is created, a controller evaluates whether
   there is sufficient room for the pod in the namespace according to the 
   pluggable policies defined in the AAQ object and the quotas enforced in 
   these namespaces.
...
For resource boundaries in AAQ quota affected by objects not requiring scheduling, 
the AAQ Operator creates and manages a ResourceQuota. This ensures changes in the
AAQ quota specifications update the managed ResourceQuota specifications,
and updates to the managed ResourceQuota status reflect back to the
AAQ quota status, keeping them in sync.

Similarly, for resource boundaries in the AAQ cluster quota affected by objects
not requiring scheduling, the AAQ Operator creates and manages a ClusterResourceQuota. This
ensures changes in the AAQ cluster quota specifications update the managed
ClusterResourceQuota specifications, and updates to the managed ClusterResourceQuota status
reflect back to the AAQ cluster quota status, maintaining synchronization.
...
pods related boundaries are allowed with AAQ cluster quota.

The primary purpose of these managed quotas is to set unified quotas for all boundaries, eliminating
the need to manage both ResourceQuota and AAQ quota, or
...
application policy, and an error if something goes wrong during the evaluation.

If a pod doesn't belong to a policy defined in the AAQ object, the policy won't be considered, or if an
error has occurred, the policy won't be considered. In other words, the returned ResourceList is
ignored.

If a pod does not belong to any policy, native quota evaluation will apply, and pod counting will be
the same as native ResourceQuota counting.

If a pod belongs to multiple policies, the evaluation of all these policies will 
apply to this pod, and the counting will be the sum of all policy evaluations.
...
* `aaq.spec.configuration.SidecarEvaluators` - Slice of standard Kubernetes container object. Adding containers to the 
SidecarEvaluators will include sidecar calculator for custom counting. The containers should mount a directory under `/var/run/aaq-sockets` that's called `sockets-dir`.

  We've implemented a [simplistic side-car container](https://github.com/Barakmor1/application-aware-quota/tree/adddoc/example_sidecars/label-sidecar) for education purposes. Let's use it as an example.
  ...
  refer to [example_sidecars/label-sidecar](https://github.com/Barakmor1/application-aware-quota/tree/adddoc/example_sidecars/label-sidecar/cmd/label-sidecar.go).
