...
for target in ${PUSH_TARGETS[@]}; do
    ...
    for arch in ${BUILD_ARCHES}; do
        ...
        ${AAQ_CRI} push "${IMAGE_NAME}"
        IMAGES="${IMAGES} ${IMAGE_NAME}"
    done

   # Can't overwrite an existing manifest. Remove it if it exists
...
for target in ${PUSH_EXAMPLE_SIDECARS[@]}; do
    ...
    for arch in ${BUILD_ARCHES}; do
      IMAGE_NAME="${MANIFEST_NAME}-${arch}"
      ${AAQ_CRI} build --platform="linux/${arch}" --build-arg git_sha=${GIT_HASH} -t ${IMAGE_NAME} . -f Dockerfile.${BIN_NAME}
      ${AAQ_CRI} push "${IMAGE_NAME}"
      IMAGES="${IMAGES} ${IMAGE_NAME}"
    done
